...
//	Program can and will account for extraneous
//	input, such as division by zero, negative
//	integers, and long whole number integers.
//
/////////////////////////////////////////////////////
...
void resetFilePointer( ifstream& inF, const char inputFile[] ); // function that resets next character captured by .get() to beginning of file
...
{
	inF.clear();											// clear eof() flag on ifstream, as it may cause while(!.eof()) to break
...
void storeNumAnalysis(ifstream& inF, ofstream& outF)	// third and final ofstream print (number analysis) function def.
{
	...
	bool isNeg = false;										// bool value to indicate the sign of int being read for proper sum calc
	...
	while (!inF.eof())
	{
		...
		{
			if ( chPrev == '-' )								// check if last character was literal dash or minus ('-')
			{
				isNeg = true;								// if so, set isNeg to true
			}
			if ( !isNeg )
			{
				currentNum = currentNum * 10 + ( ch - '0' );				// when number doesnt have '-', number = number * 10 + ( 'digit' - '0' )
			}
			else if ( isNeg )
			{
				currentNum = currentNum * 10 - ( ch - '0' );				// when number does have '-', number = number * 10 - ( 'digit' - '0' )
			}	
		}
		else if ( !isdigit(ch) && isdigit(chPrev) )						// if .get() is not digit, and last .get() IS, confirm end of number and add to sum
		{
			numTally++;
			numSum += currentNum;
			currentNum = 0;
